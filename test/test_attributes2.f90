...
subroutine test_read_attributes(path)
    ...
    print *, attr_str
    if (len_trim(attr_str) /= 48) error stop 'read_grp_attribute title'
    print *, attr_str

    if(id_group /= 1) error stop 'read_grp_attribute id_group'
...
call h%write_attribute('/', 'title', "NMC Global Product Set: Pressure at max. wind", 1)
call h%close()
...
print *, attr_str
if ( id_group /= 1) error stop 'read/write global attribute :id_group'

call h%close()

call h%open(path, action='rw')
call h%write_attribute('P', 'long_name', "pressure at maximum wind")
call h%write_attribute('/', 'title', "NMC Global Product Set: Pressure at Maximun Wind", 1)
call h%close()
